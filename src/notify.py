...
def poll_telegram_for_text(
    ...
    Returns the first stripped text message (the caller classifies it as a
    trigger keyword vs a 6-digit code). Returns ``(text, new_offset)`` where
    ``new_offset`` is that message's update_id, so the next poll continues to
    the following message -- important when a keyword and the code arrive
    close together. Non-text / foreign-chat updates are skipped and advance
    the offset past them. Network / API failures are logged + swallowed,
    returning ``(None, offset)`` so the caller keeps trying.
    """
